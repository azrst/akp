...
export class D_LocalNews extends Component {
    constructor(props) {
        super(props)
        this.state = {
            localNewsData: this.props.NewsLocal,
            headerImageUrl: '',
            titleImageUrl: '',
            linkImageUrl: '',
        }
    ...
    componentDidMount() {
        // if (this.props.NewsLocal.length == 0) {
    ...
    serviceNewsLocal = () => {
        ...
        fetch(link.NewsLinkQueryLocal + 'covid' + '&country=' + this.props.myAlpha2Code + link.NewsKey, {
            method: 'GET',
            // header : {},
            // body : {}
        })
    ...
    NewsRowRender() {
        if (this.props.NewsLocal.length != 0) {
            return (
                <View>
                    {this.state.localNewsData.map((item, index) => {
                        return (
...
export default connect(mapStateToProps, mapDispatchToProps)(D_LocalNews)